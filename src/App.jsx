...
function WeatherView({ weather, loading, err, updated }) {
  const [selDay,setSelDay]=useState(()=>{const ti=weather.findIndex(d=>d.isToday);return ti>=0?ti:0;});
  ...
  useEffect(()=>{
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${LAT}&longitude=${LON}&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max,windspeed_10m_max,windgusts_10m_max,weathercode&temperature_unit=fahrenheit&windspeed_unit=mph&timezone=America%2FNew_York&forecast_days=7`)
      ...
      .then(({daily:d})=>{
        ...
        setWeather(days);
        setWxUpdated(new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}));
        setWxLoading(false);
      })
      .catch(e=>{setWxErr(e.message);setWxLoading(false);});
  },[]);
  ...
  return (
    <div style={{minHeight:"100vh",background:`linear-gradient(180deg,#0e1a08 0%,#0a0e06 60%,#090c05 100%)`,color:T.parchment,fontFamily:FONT.body,paddingBottom:48}}>
        ...
        <div style={{maxWidth:740,margin:"0 auto",position:"relative"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
            <div>
              ...
              <div style={{fontSize:12,color:T.mist,marginTop:4,fontFamily:FONT.body,letterSpacing:"0.3px"}}>Weather · Events · Sports · Breweries · Fitness</div>
            </div>
      ...
      <div style={{maxWidth:740,margin:"0 auto",padding:"0 16px"}}>
        {activeTab==="weather"    && <WeatherView weather={weather} loading={wxLoading} err={wxErr} updated={wxUpdated}/>}
        {activeTab==="events"     && <EventsView/>}
        {activeTab==="sports"     && <SportsView/>}
        {activeTab==="breweries"  && <BreweriesView/>}
        {activeTab==="fitness"    && <FitnessView weather={weather}/>}
      </div>
    </div>
  );
}
